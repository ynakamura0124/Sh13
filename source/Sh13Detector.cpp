...
void Sh13Detector::ConstructDetector(LENSLongDetectorConstruction* mainDetectorConstruction) {
	...
	/* modify YSO placement */
	{
		...
		auto boxMaterial = ((Sh13DetectorConstruction*)mainDetectorConstruction)->GetPLA();
	...
	/* placement of the source container if PointGammaSourceGeneratorAction will be used */
	{
		...
		if (yaml_reader.GetString("PrimaryGeneratorAction") == "PointGammaSourceGeneratorAction") {
			...
			std::cout << "[Sh13Detector.cpp]: Placement of source: (" << pos_x << "," << pos_y << "," << pos_z << ")" << std::endl;
